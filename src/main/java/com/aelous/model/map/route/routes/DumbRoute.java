...
public class DumbRoute {
    ...
    public static void route(Entity entity, int destX, int destY) {
        entity.resetSteps();
        if (entity.isMovementBlocked(false, false)) return;
        ...
        while (true) {
            Direction stepDir =
                    getStepDirection(
                            entity.getRouteFinder().getClipUtils(), x, y, z, size, destX, destY);
            if (stepDir == null) return;
            x += stepDir.deltaX;
            y += stepDir.deltaY;
            if (!entity.addStep(x, y) || (x == destX && y == destY)) return;
        }
    }
    ...
    public static Direction getDirection(
        ...
        else if (x > destX) west = true;

        boolean south = false, north = false;
        if (y < destY) north = true;
    ...
    public static Direction getStepDirection(
        ...
        if (west) {
            if (south) {
                ...
                if (allowEntrance(clipping, x, y, z, size, Direction.SOUTH)) return Direction.SOUTH;
            } else if (north) {
                if (allowEntrance(clipping, x, y, z, size, Direction.NORTH_WEST))
                    return Direction.NORTH_WEST;
                if (allowEntrance(clipping, x, y, z, size, Direction.NORTH)) return Direction.NORTH;
            }
            return allowEntrance(clipping, x, y, z, size, Direction.WEST) ? Direction.WEST : null;
        }
        if (east) {
    ...
    private static boolean allowEntrance(
            ClipUtils clipUtils, int x, int y, int z, int size, Direction dir) {
        int dx = dir.deltaX;
        int dy = dir.deltaY;
        if (size == 1) {
            if (dx == -1 && dy == 0 && (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0)
                return true;
            if (dx == 1 && dy == 0 && (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0) return true;
            ...
            if (dx == -1
                    && dy == -1
                    && (clipUtils.getAbs(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0
                    && (clipUtils.getAbs(x, y - 1, z) & SOUTH_MASK) == 0) return true;
            if (dx == 1
                    && dy == -1
                    && (clipUtils.getAbs(x + 1, y - 1, z) & SOUTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0
                    && (clipUtils.getAbs(x, y - 1, z) & SOUTH_MASK) == 0) return true;
            if (dx == -1
                    && dy == 1
                    && (clipUtils.getAbs(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0
                    && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0) return true;
            if (dx == 1
                    && dy == 1
                    && (clipUtils.getAbs(x + 1, y + 1, z) & NORTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0
                    && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0) return true;
        } else if (size == 2) {
            if (dx == -1
                    && dy == 0
                    && (clipUtils.getAbs(x - 1, y, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0) return true;
            if (dx == 1
                    && dy == 0
                    && (clipUtils.getAbs(x + 2, y, z) & SOUTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(x + 2, y + 1, z) & NORTH_EAST_MASK) == 0) return true;
            if (dx == 0
                    && dy == -1
                    && (clipUtils.getAbs(x, y - 1, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x + 1, y - 1, z) & SOUTH_EAST_MASK) == 0) return true;
            if (dx == 0
                    && dy == 1
                    && (clipUtils.getAbs(x, y + 2, z) & NORTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x + 1, y + 2, z) & NORTH_EAST_MASK) == 0) return true;
            if (dx == -1
                    && dy == -1
                    && (clipUtils.getAbs(x - 1, y, z) & 0x124013e) == 0
                    && (clipUtils.getAbs(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x, y - 1, z) & 0x124018f) == 0) return true;
            if (dx == 1
                    && dy == -1
                    && (clipUtils.getAbs(x + 1, y - 1, z) & 0x124018f) == 0
                    && (clipUtils.getAbs(x + 2, y - 1, z) & SOUTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(x + 2, y, z) & 0x12401e3) == 0) return true;
            if (dx == -1
                    && dy == 1
                    && (clipUtils.getAbs(x - 1, y + 1, z) & 0x124013e) == 0
                    && (clipUtils.getAbs(x - 1, y + 2, z) & NORTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x, y + 2, z) & 0x12401f8) == 0) return true;
            if (dx == 1
                    && dy == 1
                    && (clipUtils.getAbs(x + 1, y + 2, z) & 0x12401f8) == 0
                    && (clipUtils.getAbs(x + 2, y + 2, z) & NORTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(x + 2, y + 1, z) & 0x12401e3) == 0) return true;
        } else {
            if (dx == -1
                    && dy == 0
                    && (clipUtils.getAbs(x - 1, y, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x - 1, y + size - 1, z) & NORTH_WEST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(x - 1, i + y, z) & 0x124013e) != 0) return false;
                }
                return true;
            }
            if (dx == 1
                    && dy == 0
                    && (clipUtils.getAbs(x + size, y, z) & SOUTH_EAST_MASK) == 0
                    && (clipUtils.getAbs(size + x, size + y - 1, z) & NORTH_EAST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(size + x, y + i, z) & 0x12401e3) != 0) return false;
                }
                return true;
            }
            if (dx == 0
                    && dy == -1
                    && (clipUtils.getAbs(x, y - 1, z) & SOUTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(size + x - 1, y - 1, z) & SOUTH_EAST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(x + i, y - 1, z) & 0x124018f) != 0) return false;
                }
                return true;
            }
            if (dx == 0
                    && dy == 1
                    && (clipUtils.getAbs(x, y + size, z) & NORTH_WEST_MASK) == 0
                    && (clipUtils.getAbs(x + size - 1, size + y, z) & NORTH_EAST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(i + x, y + size, z) & 0x12401f8) != 0) return false;
                }
                return true;
            }
            if (dx == -1
                    ...
                    && (clipUtils.getAbs(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(x - 1, i + (y - 1), z) & 0x124013e) != 0
                            || ((clipUtils.getAbs(i + (x - 1), y - 1, z) & 0x124018f) != 0))
                        return false;
                }
                return true;
            }
            if (dx == 1
                    && dy == -1
                    && (clipUtils.getAbs(x + size, y - 1, z) & SOUTH_EAST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(size + x, y - 1 + i, z) & 0x12401e3) != 0
                            || (clipUtils.getAbs(i + x, y - 1, z) & 0x124018f) != 0) return false;
                }
                return true;
            }
            if (dx == -1
                    && dy == 1
                    && (clipUtils.getAbs(x - 1, size + y, z) & NORTH_WEST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(x - 1, i + y, z) & 0x124013e) != 0
                            || (clipUtils.getAbs(x - 1 + i, size + y, z) & 0x12401f8) != 0)
                        return false;
                }
                return true;
            }
            if (dx == 1
                    && dy == 1
                    && (clipUtils.getAbs(size + x, size + y, z) & NORTH_EAST_MASK) == 0) {
                for (int i = 1; i < size - 1; i++) {
                    if ((clipUtils.getAbs(x + i, size + y, z) & 0x12401f8) != 0
                            || (clipUtils.getAbs(x + size, y + i, z) & 0x12401e3) != 0)
                        return false;
                }
                return true;
            }
