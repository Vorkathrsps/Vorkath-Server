        ...
        if (option == 1) {
            ...
            if (obj.getId() == BELL_21394) {
                if (player.getTimers().has(TimerKey.RISK_FIGHT_BELL)) {
                    ...
                    World.getWorld().sendWorldMessage("<lsprite=2011>" + Color.BLUE.wrap(player.getUsername()) + Color.RAID_PURPLE.wrap(" has just rung the bell at the risk zone and is looking for a fight!"));
                }
                return true;
            }
