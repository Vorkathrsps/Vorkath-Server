    ...
    private static void open(Player player, int key) {
        ...
        Chain.bound(player).runFn(1, () -> {
            ...
            for (int i = 0; i < roll; i++) {
                ...
                World.getWorld().sendWorldMessage("<lsprite=2010><lsprite=2016>[<col=" + Color.MEDRED.getColorValue() + ">Larran's chest</col>]: " + "<col=1e44b3>" + player.getUsername() + " has just looted the Larran's chest with a Larran's key!");
                ...
                if (reward.getValue() >= 30_000) {
                    ...
                    String msg = "<lsprite=2010>[<col=" + Color.MEDRED.getColorValue() + ">Larran's chest</col>]: " + "<col=1e44b3>" + player.getUsername() + " has received " + amtString + " " + reward.unnote().name() + "!";
