...
public class Mine extends PacketInteraction {
    ...
    private static void mine(Player player, Ore rockType, int replId) {
        ...
        Optional<Pickaxe> pick = Mine.findPickaxe(player);
        ...
        player.action.execute(new UnwalkableAction(player, pick.get().getDelay()) {
            @Override
            protected void execute() {
                ...
                if (player.getAttribOr(AttributeKey.DEBUG_MESSAGES, false)) {
                    player.debug("Mining chance: " + success + ".");
                }
