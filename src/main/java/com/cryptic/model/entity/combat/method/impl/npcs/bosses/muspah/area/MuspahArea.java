package com.cryptic.model.entity.combat.method.impl.npcs.bosses.muspah.area;
...
public class MuspahArea extends Controller {
    public static Area ROOM = new Area(Tile.regionToTile(11330).getX(), Tile.regionToTile(11330).getY(), Tile.regionToTile(11330).getX() + 63, Tile.regionToTile(11330).getY() + 63);

    public MuspahArea() {
        super(Collections.singletonList(new Area(Tile.regionToTile(11330).getX(), Tile.regionToTile(11330).getY(), Tile.regionToTile(11330).getX() + 63, Tile.regionToTile(11330).getY() + 63)));
    }
    ...
    public void enter(Player player) {
        for (var regions : player.getRegions()) {
            for (var npc : regions.getNpcs()) {
                if (npc.id() == 12078 || npc.id() == 12076) {
                    if (npc.getZ() != player.getZ()) continue;
                    if (!npc.dead()) {
                        HealthHud.open(player, HealthHud.Type.REGULAR, "Phantom Muspah", npc.hp());
    ...
    public boolean inside(Entity entity) {
        return entity.getAsPlayer().getMuspahInstance() != null && ROOM.transformArea(0, 0, 0, 0, entity.getAsPlayer().getMuspahInstance().getzLevel()).contains(entity.tile());
    }
}
