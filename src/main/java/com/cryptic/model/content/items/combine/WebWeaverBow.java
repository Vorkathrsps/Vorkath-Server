...
public class WebWeaverBow extends PacketInteraction {
    int[] items = new int[]{ItemIdentifiers.FANGS_OF_VENENATIS, ItemIdentifiers.CRAWS_BOW};
    @Override
    public boolean handleItemOnItemInteraction(Player player, Item use, Item usedWith) {
        ...
        if (ArrayUtils.contains(items, use.getId())) {
            if (ArrayUtils.contains(items, usedWith.getId())) {
                ...
                player.getInventory().add(new Item(ItemIdentifiers.WEBWEAVER_BOW));
                return true;
            }
        }
        return false;
