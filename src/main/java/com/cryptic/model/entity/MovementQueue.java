...
public class MovementQueue {
    ...
    public void interpolate(int destX, int destY, StepType stepType, int maxSteps) {
        ...
        while (true) {
            com.cryptic.model.map.route.Direction stepDir = getStepDirection(ClipUtils.NONE, x, y, z, size, destX, destY);
            if (stepDir == null)
                return;
            x += stepDir.deltaX;
            y += stepDir.deltaY;
    ...
    public static boolean dumbReachable(int destX, int destY, Tile start) {
        ...
        var maxSteps = 0;
        while (true) {
            com.cryptic.model.map.route.Direction stepDir = getStepDirection(ClipUtils.REGULAR, x, y, z, size, destX, destY);
    ...
    public boolean isRunning() {
        if(entity.isPlayer()) {
            return entity.getAttribOr(AttributeKey.IS_RUNNING, false);
        }
        return running;
    }
