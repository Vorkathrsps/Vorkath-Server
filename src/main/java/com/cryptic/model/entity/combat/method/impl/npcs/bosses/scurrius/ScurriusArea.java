...
public class ScurriusArea extends Controller {

    public static final Area ROOM = new Area(3289, 9859, 3309, 9877);
    public ScurriusArea() {
        super(Collections.singletonList(new Area(3287, 9856, 3309, 9878)));
    }
    ...
    public void enter(Player player) {
        for (var regions : player.getRegions()) {
            for (var npc : regions.getNpcs()) {
                if (npc.id() == 7221) {
                    if (!npc.dead()) {
                        HealthHud.open(player, HealthHud.Type.REGULAR, "Scurrius", npc.hp());
