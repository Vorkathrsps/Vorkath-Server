...
public class PurpleSlayerHelmetImbued  extends PacketInteraction {
    int[] items = new int[]{ItemIdentifiers.PURPLE_SLAYER_HELMET, ItemIdentifiers.SCROLL_OF_REDIRECTION};
    @Override
    public boolean handleItemOnItemInteraction(Player player, Item use, Item usedWith) {
        ...
        if (ArrayUtils.contains(items, use.getId())) {
            if (ArrayUtils.contains(items, usedWith.getId())) {
                ...
                var def = ItemDefinition.getInstance(ItemIdentifiers.PURPLE_SLAYER_HELMET_I);
