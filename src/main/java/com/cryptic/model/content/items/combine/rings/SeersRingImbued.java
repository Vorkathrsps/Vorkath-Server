...
public class SeersRingImbued extends PacketInteraction {
    int[] items = new int[]{ItemIdentifiers.SEERS_RING, ItemIdentifiers.SCROLL_OF_REDIRECTION};
    @Override
    public boolean handleItemOnItemInteraction(Player player, Item use, Item usedWith) {
        ...
        if (ArrayUtils.contains(items, use.getId())) {
            if (ArrayUtils.contains(items, usedWith.getId())) {
                ...
                player.getInventory().add(new Item(ItemIdentifiers.SEERS_RING_I));
            }
        }
        return false;
    }
