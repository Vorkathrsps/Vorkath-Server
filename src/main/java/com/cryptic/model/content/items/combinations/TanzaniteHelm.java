...
public class TanzaniteHelm extends PacketInteraction {
    int[] items = new int[]{ItemIdentifiers.TANZANITE_MUTAGEN, ItemIdentifiers.SERPENTINE_HELM};
    @Override
    public boolean handleItemOnItemInteraction(Player player, Item use, Item usedWith) {
        ...
        if (ArrayUtils.contains(items, use.getId())) {
            if (ArrayUtils.contains(items, usedWith.getId())) {
                ...
                player.getInventory().add(new Item(ItemIdentifiers.TANZANITE_HELM));
                return true;
            }
        }
        return false;
