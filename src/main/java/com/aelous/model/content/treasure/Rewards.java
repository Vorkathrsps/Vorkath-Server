        ...
        if (World.getWorld().rollDie(2, 1)) {
            ...
            if (reward != null) {
                ...
                if (reward.reward.getValue() >= 30000 && !player.getUsername().equalsIgnoreCase("Box test"))
                    World.getWorld().sendWorldMessage("<img=1081><col=0052cc>" + player.getUsername() + " has just received <col=" + Color.HOTPINK.getColorValue() + ">" + Utils.getAOrAn(reward.reward.name()) + " " + reward.reward.name() + " <col=0052cc>from a master casket!");
            }
        }
    }
