...
public class LootKey {
    ...
    public static LootKey[] filteredKeys(LootKey[] keys) {
    ...
    public static LootKey[] filteredKeys(LinkedList<LootKey> keyList) {
     ...
     */
    public static LootKeyContainers infoForPlayer(Player player) {
    ...
    public static void handleDeath(Player dead, Player killer) {
        ...
        if (!killer.deadRecently()) {
            ...
            var deadInfo = infoForPlayer(dead).updateLootKey(killer);
    ...
    public static boolean lootKeysEnabled(Player killer) {
     ...
     */
    private static void giveOrDropDeadLootKey(Player dead, Player killer, LootKeyContainers deadInfo, LootKeyContainers killerInfo, int carried) {
        //TODO when a player dies, don't send a loot key but drop the loot to the ground instead.
        //TODO including all the loot from the loot keys they was carrying
        if(deadInfo.lootKeyItems != null && deadInfo.lootKeyItems.lootContainer != null && deadInfo.lootKeyItems.lootContainer.containerValue() <= 0) {
            return;
        }
    ...
    private static void handleDangerousZoneDeath(Player dead, Player killer, LootKeyContainers deadInfo, LootKeyContainers killerInfo) {
            ...
            if (toAdd > 1) {
                ...
                while (toAdd > 0 && killer.inventory().getFreeSlots() > 0) {
                    ...
                    if (!killer.inventory().isFull()) {
                        if(killer.getGameMode().isDarklord()) {
     ...
     */
    public static void pickupKey(Player player, GroundItem item) {
